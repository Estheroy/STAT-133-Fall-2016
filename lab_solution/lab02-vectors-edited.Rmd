...
``` {r}
# OH 
'http://www.when2meet.com/?5424066-XRXIs'
...
```{r}
objects = data.frame(area,capital, seats, state,water)
lapply(X = objects, MARGIN = 2,FUN = head)
```
...
y <- 'timo'
class(y) # character vector
mode(y) # what is contained
x <- 1:16
class(x)
mode(x)
dim(x)
...
x <- factor(x, ordered = TRUE) # levels: categorical(character) variables in R
x 
level = levels(x)
level
class(x)
...
```{r}
capital[which(state == 'California')]
...
``` {r}
total["Alabama"] #can't call by the elements, only by indices or names
...
```{r, eval = FALSE}
plot(log_area, log_water)
text(log_area, log_water, labels = abbreviate(state)) # 4 characters
```
...
area_square_miles <- 0.386*area
area_square_miles
water_square_miles <- 0.386*water
water_square_miles
...
plot(log(area_square_miles), log(water_square_miles))
