---
title: "Lab 3: Reading Data Tables"
...
# download.file(origin, "~/Desktop/timo"")
```
...
url2 <- "http://archive.ics.uci.edu/ml/machine-learning-databases/bridges/bridges.data.version1"
download.file(url2, 'bridges.data')
...
bridges <- read.table('bridges.data', sep = ',', col.names = col_names2)
bridges[bridges == '?'] <- NA


col_types2[which(col_types2 == 'integer')] <- 'numeric'
bridges <- read.table('bridges.data', sep = ',', col.names = col_names2, na.strings = '?', colClasses = col_types2)

# stringsAsFactors = FALSE
bridges_omit <- na.omit(bridges)
rownames(bridges_omit) <- NULL
rownames(bridges_omit)

rownames(bridges_omit) <- seq(length = nrow(bridges_omit))
rownames(bridges_omit)
# data lost >1/2
bridges_omit
...
```{r}
# your code
bridges_csv <- read.csv('bridges.data', col.names = col_names2)
nrow(bridges_csv) == nrow(bridges)

# Look at ?read.table vs. ?read.csv
bridges_csv <- read.csv('bridges.data', col.names = col_names2, header = FALSE)
nrow(bridges_csv) == nrow(bridges)

bridges_csv <- read.csv('bridges.data', col.names = col_names2, header = FALSE, na.strings = '?', colClasses = col_types2)
